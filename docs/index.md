---
icon: lucide/footprints
---

# Добре дошли

Ние сме група ентусиасти, които силно вярваме, че движението е в основата на едно
усмихнато ежедневие. То води, някак естествено, към по-отворена комуникация и динамичен
начин на живот. Нашата цел е да помогнем за създаването на по-здраво и щастливо общество
чрез едно от най-присъщите ни неща &mdash; *ходенето*.

??? info "Цветница е!"

	<div style="float: left; margin: 0 10px 1px 0;">
		<img src="../img/цветница.jpg" alt="Цветница" style="max-width:60px; height:auto;">
	</div>

	Денят, в който празнуваме красотата на природата и символиката на цветята. Нека домове и сърца ни се изпълнят с радост и цвят! 
	Да излезем навън, да се разходим и да оцветим мислите си ...

<div class="grid cards" markdown>
- :lucide-group: __[НПО]__ – Пешачко е сдружение в обществена полза. Ние насърчаваме
  ходенето и активния начин на живот.
- :lucide-users: __[Събития]__ – Всяко събите е възможност за промяна &mdash; а колко ни
  е необходима тя!
- :lucide-house: __[Стихове по пътеките]__ – Нашите членове споделят своите поетични
  мигове по пътеките. Ако искаш, включи се и ти!
- :girl::boy: __[Към децата]__ –
  Здравейте!  
  Пешачко аз се казвам  
  и мисията ми е да опазвам -  
  природата и въздуха най-вече  
  в желанието си да виждам всичко надалече!
  [:fontawesome-solid-angles-right:{.custom-icon-size}](poems/20260105_antoaneta_marinova.md)
</div>

<section style="text-align: center;">
``` mermaid
flowchart TB
    A1(Подари чист въздух)
    A2(Инвестирай в здраве)

    C(Ходи пеша)

    B1(Създай общност)
    B2(Изгради навици)

    A1 ~~~ C --> A1
    A2 ~~~ C --> A2
    C --> B1
    C --> B2

	style C stroke-width:3px
	style A2 stroke:lightgreen, stroke-width:3px
	style A1 stroke:#B3E5FC, stroke-width:3px
	style B1 stroke:#FFCDD2, stroke-width:3px
	style B2 stroke:#FFD580, stroke-width:3px
```
</section>

Това е нашата страница. Тук ще намерите информация за минали и планирани събития,
както и кратки статии на теми които са ни мотивирали по някакъв начин.

Всяка стъпка има значение ...

<figure markdown="span">
  ![Image title](img/peshachko.png){ width="300" }
</figure>

[НПО]: ngo.md
[Нашият град]: environment.md
[Събития]: events/events.md
[Стихове по пътеките]: poems/poems.md
[Към децата]: poems/20260105_antoaneta_marinova
